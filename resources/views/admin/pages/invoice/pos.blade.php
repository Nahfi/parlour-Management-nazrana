...
<div class="page-content">
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            {{-- Original Design START --}}
            <section id="invoice-print-pos80mm">
            <div id="invoice-main"
                style="font-family: sans-serif; width:235px; min-height:377px; margin: 0 auto;text-transform:capitalize;color:#000; padding:5px;background: #fff;overflow: hidden;border: 1px dotted #000;">
                <div style="min-height:357px;font-size:11px;background: #fff;margin: 0 auto;">
                    ...
                    <div class="invoice-info" style="font-size: 8px;">
                        <div>
                            <div style="width: 100%; text-align: center;">
                                <p
                                    style="margin-bottom: 5px;margin-top: 5px;border: 1px dashed #000;font-size:11px;display: inline-block;font-weight: 500;padding: 2px;">
                                    Invoice No: {{ $invoice->invoice_id }}</p>
                            </div>
                            <div style="width: 100%;text-align: start;">
                                <div style="display: flex;">
                                    <div style="width: 65%;">
                                        <p style="margin-bottom: 0;margin-top: 5px;">Customer:{{ $invoice->customer->name,10 }}</p>
                                    </div>
                                    <div style="width: 35%;">
                                        <p style="margin-bottom: 0;margin-top: 5px;">Phone: {{$invoice->customer->phone}}</p>
                                    </div>
                                </div>
                            </div>
                            <div style="width: 100%;text-align: start;">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th
                                        ...
                                        Service</th>
                                    <th style="text-align: end;border-top: 1px dashed #000;border-bottom: 1px dashed #000;padding-right: 5px;">
                                        Price</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @foreach ($invoiceInfo as $invoices )
                                    <tr>
                                        <td>1</td>
                                        <td style="max-width: 100px;">{{ $invoices->service->name }}</td>
                                        <td style="text-align: end;">{{ $invoices->price }}</td>
                                    </tr>
                                @endforeach
                        ...
                        <table style="text-align: end;margin-left: auto">
                            <tbody>
                                <tr>
                                    <td>Sub Total</td>
                                    <td>:</td>
                                    <td>{{$invoice->subtotal}}<span>&#2547;</span></td>
                                </tr>
                                <tr>
                                    <td>Discount(-)</td>
                                    <td>:</td>
                                    <td>
                                        {{$invoice->discount}} @if ($invoice->discountType == '%')
                                            <span>&#37;</span>
                                        @else
                                            <span>&#2547;</span>
                                        @endif
                                ...
                                <tr>
                                    <td style="border-bottom: 0.5px dashed #000;">Tax(+)</td>
                                    <td style="border-bottom: 0.5px dashed #000;">:</td>
                                    <td style="border-bottom: 0.5px dashed #000;">{{$invoice->tax}}<span>&#37;</span></td>
                                </tr>
                                <tr>
                                    <th>Grand Total</th>
                                    <td>:</td>
                                    <th>{{$invoice->grandtotal}}<span>&#2547;</span></th>
                                </tr>
                                <tr>
                                    <td>Amount Paid</td>
                                    <td>:</td>
                                    <td>{{$invoice->amountPaid}}<span>&#2547;</span></td>
                                </tr>
                                <tr>
                                    <td>Amount Due</td>
                                    <td>:</td>
                                    <td>{{$invoice->totalDue}}<span>&#2547;</span></td>
                                </tr>
                                <tr>
                                    <td>Payment Type</td>
                                    <td>:</td>
                    ...
                    <div class="pos-footer" style="border-top: 0.5px dashed #000;margin-top: 10px;margin-bottom: 10px;">
                        <h1 style="text-align: center; font-size: 11px;padding-top:8px;margin-bottom: 0;color:#000">Thanks For Visiting
                            Us</h1>
                    </div>
                </div>
            </div>
            ...
            @foreach ($invoiceInfo as $invoices)
            <div id="token-{{ $loop->iteration }}"
                style="font-family: sans-serif; width:235px; margin: 0 auto;text-transform:capitalize;color:#000; padding:10px;background: #fff;overflow: hidden;border: 1px dotted #000;">
                <div style="font-size:11px;background: #fff;margin: 0 auto;">
                    <div class="invoice-info">
                        <div>
                            <div style="width: 100%; text-align: center;">
                                <p
                                    style="margin:2px;display: inline-block;font-weight: 700; font-size: 20px;">
                                    Service Token</p>
                            </div>
                            <div style="width: 100%; text-align: center;">
                                <p
                                    style="margin:2px;border: 1px dashed #000;display: inline-block;font-weight: 500;padding: 2px;">
                                    Invoice No: {{ $invoice->invoice_id }}</p>
                            </div>
                            <div style="width: 100%;text-align: center;">
                                <div>
                                    <div>
                                        <p style="margin-bottom: 0;margin-top: 5px;font-weight: bold;">{{ $invoices->service->name }}</p>
                                    </div>
                                    <div>
                                        <p style="margin-bottom: 0;margin-top: 5px;">Employee: {{ $invoices->employee->name }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
$(document).on('click','#print',function(e){
   ...
   @foreach ($invoiceInfo as $invoices )
    $("#token-{{ $loop->iteration }}").printThis({
            pageTitle: "",
            header: null,
            footer: null,
            margin: null,
